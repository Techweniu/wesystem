...
// Importaremos as novas ações que criaremos na próxima etapa
...
interface MarkServiceReceivedButtonProps {
  serviceId: string
  clientId: string // Necessário para revalidar a página do cliente
  expectedAmount: number
  isReceived: boolean
}

export function MarkServiceReceivedButton({
  serviceId,
  clientId,
  expectedAmount,
  isReceived,
}: MarkServiceReceivedButtonProps) {
  ...
  const handlePayment = () => {
    ...
    startTransition(async () => {
      ...
      if (result.error) {
        toast.error("Erro ao registrar recebimento", { description: result.error })
      } else {
  ...
  const handleUndo = () => {
    if (!confirm("Tem certeza que deseja reverter este recebimento? O status do serviço voltará para 'Pendente'.")) {
      return
    }
