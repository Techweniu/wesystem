...
// Função para buscar os detalhes do colaborador no Supabase
async function getEmployeeDetails(id: string) {
  ...
  // Gera URLs de download para os contratos
  if (employee && employee.employee_contracts) {
    for (const contract of employee.employee_contracts) {
      if (contract.storage_path) {
        const { data } = await supabase.storage.from('employee_contracts').createSignedUrl(contract.storage_path, 60 * 60); // URL válida por 1 hora
...
// Função para buscar todos os colaboradores (para o formulário de edição)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent><Badge variant={employee.status === "active" ? "default" : "outline"}>{employee.status === "active" ? "Ativo" : "Inativo"}</Badge></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Custo Mensal (Salário)</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(employee.salary || 0)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Tempo de Casa</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold">{timeSinceHired}</div></CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
            <CardHeader className="flex flex-row items-center justify-between">
                ...
                <EditEmployeeForm employee={employee} allEmployees={allEmployees}><Button variant="outline" size="sm">Editar</Button></EditEmployeeForm>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
                ...
                <div className="flex items-center gap-2"><Calendar className="h-4 w-4 text-muted-foreground" /><p>Data de Contratação: {format(hireDate, "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}</p></div>
                <div className="flex items-center gap-2"><Mail className="h-4 w-4 text-muted-foreground" /><p>{employee.email}</p></div>
                        ...
                        <AccordionItem value={obs.id} key={obs.id}>
                            <AccordionTrigger>
                                <div className="flex justify-between items-center w-full pr-4">
                                    ...
                                    {obs.tag === 'positive' ? (
                                        <div className="flex items-center gap-1.5 text-xs font-medium text-green-500"><ThumbsUp className="h-4 w-4" /> Positiva</div>
                                    ) : (
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2"><Award className="h-5 w-5" /> Contribuições e Benefícios</CardTitle>
            ...
            <Table>
                ...
                <TableBody>
                    {employee.employee_contributions.length > 0 ? employee.employee_contributions.map((item) => (
                        ...
                        <TableRow>
                            <TableCell colSpan={4} className="h-24 text-center">Nenhuma contribuição registrada.</TableCell>
                        </TableRow>
                    )}
                </TableBody>
            </Table>
                ...
                {employee.payment_day ? (
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div>
                            ...
                            <p className="font-medium">{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(employee.salary || 0)}</p>
                        </div>
                        <MarkPaymentButton
                    ...
                    <p className="text-sm text-muted-foreground text-center">O dia de pagamento ainda não foi definido para este colaborador.</p>
                )}
            </div>
             ...
             <Table>
                ...
                <TableBody>
                    {employee.employee_payments.length > 0 ? employee.employee_payments.map((payment) => (
                        ...
                        <TableRow>
                            <TableCell colSpan={2} className="h-24 text-center">Nenhum pagamento registrado.</TableCell>
                        </TableRow>
                    )}
                </TableBody>
            </Table>
