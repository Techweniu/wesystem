...
async function getTeamData({ name, status }: { name?: string; status?: string }) {
  ...
  const { data: allEmployeesData } = await supabase.from("employees").select('status, salary, employee_payments(amount)');
  ...
  const totalSalary = allEmployeesData?.filter((e) => e.status === "active").reduce((sum, e) => sum + Number(e.salary || 0), 0) || 0
  const totalPaid = allEmployeesData?.flatMap(e => e.employee_payments).reduce((sum, p) => sum + Number(p.amount), 0) || 0;
