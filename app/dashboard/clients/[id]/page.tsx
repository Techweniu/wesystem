...
import { Mail, Phone, Calendar, Download, FileText, Building, MapPin, AlertTriangle, Star, History, BarChart2, ThumbsDown } from "lucide-react"; // Adicionado ThumbsDown e BarChart2
...
async function getClientDetails(id: string) {
  ...
  const { data: client, error } = await supabase
    .from("clients")
    // ATUALIZADO: Incluindo os novos campos de tráfego
    .select(`*, has_traffic_service, ad_account_organized, ads_running, contracts(*), one_time_services(*), nps_responses(*), client_contacts(*)`)
    .eq("id", id)
...
export default async function ClientDetailPage({ params }: { params: { id: string } }) {
  ...
  const getNpsBadgeVariant = (nps: number | undefined): 'destructive' | 'secondary' | 'default' | 'outline' => {
    ...
    if (nps <= 7) return 'destructive'; // Atualizado para incluir 7
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
            <div>
             ...
             {/* Botão Editar Infos agora usa o EditClientInfoForm */}
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
             ...
             {/* ====> NOVO CARD DE TRÁFEGO PAGO <==== */}
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2 text-base">
                       {/* Ícone pode ser alterado (ex: BarChartBig, Target, Megaphone) */}
                ...
                <CardContent className="space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">Cliente possui Tráfego?</span>
                        <Badge variant={client.has_traffic_service ? "default" : "outline"}>
                            {client.has_traffic_service ? "Sim" : "Não"}
                        </Badge>
                    </div>
                    <Separator />
                    <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">Conta de Anúncios Organizada?</span>
                        <Badge variant={client.ad_account_organized ? "default" : "outline"}>
                            {client.ad_account_organized ? "Sim" : "Não"}
                        </Badge>
                    </div>
                     <Separator />
                    <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">Anúncios Rodando Atualmente?</span>
                        <Badge variant={client.ads_running ? "default" : "outline"}>
                            {client.ads_running ? "Sim" : "Não"}
                        </Badge>
                    </div>
                </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        ...
                        {/* Botão de editar movido para o topo da página */}
                    </CardHeader>
                    <CardContent className="space-y-3 text-sm">
