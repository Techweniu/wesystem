...
const npsSchema = z.object({
  ...
  "Conteúdos e Roteiros": z.coerce.number().min(0).max(10), "Audiovisual": z.coerce.number().min(0).max(10), "Edição de Vídeos": z.coerce.number().min(0).max(10), "Design": z.coerce.number().min(0).max(10), "Atendimento Assessor": z.coerce.number().min(0).max(10), "Atendimento VideoMaker": z.coerce.number().min(0).max(10), "Comunicação e Presença": z.coerce.number().min(0).max(10), "Resultado da Parceria": z.coerce.number().min(0).max(10),
...
export async function addNpsResponse(formData: FormData) {
  ...
  if (!validatedFields.success) { return { error: "Dados inválidos. Todas as notas de 0 a 10 são obrigatórias." }; }
...
export async function updateClientNotes(formData: FormData) {
    ...
    if (!validatedFields.success) { return { error: "Dados inválidos para atualizar as notas." }; }
    const { clientId, client_notes, objectives } = validatedFields.data;
    const supabaseAdmin = createAdminClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_KEY!);
    const { error } = await supabaseAdmin.from("clients").update({ client_notes: client_notes || null, objectives: objectives || null, }).eq('id', clientId);
...
export async function updateServiceStatus(data: { serviceId: string, clientId: string, status: "pending" | "completed" | "cancelled" }) {
  ...
  if (!validatedFields.success) { return { error: "Dados inválidos para atualizar o status." }; }
...
const updateContractSchema = z.object({ contractId: z.string().uuid(), name: z.string().min(3, "O nome do contrato é obrigatório."), valor_mensal: z.coerce.number().min(0).optional(), start_date: z.string().min(1, "A data de início é obrigatória."), end_date: z.string().optional().or(z.literal('')), status: z.enum(['active', 'inactive']), });
export async function updateContract(formData: FormData) {
    const rawData = Object.fromEntries(formData);
    const validatedFields = updateContractSchema.safeParse(rawData);
    if (!validatedFields.success) { const firstError = Object.values(validatedFields.error.flatten().fieldErrors)[0]?.[0]; return { error: firstError || "Dados inválidos." }; }
